...
void ggNtuplizer::fillElectrons(const edm::Event &e, const edm::EventSetup &es, math::XYZPoint &pv) {
  ...
  for (edm::View<pat::Electron>::const_iterator iEle = electronHandle->begin(); iEle != electronHandle->end(); ++iEle) {
    ...
    double scale = correctionRetriever_.ScaleCorrection(runNumber, iEle->isEB(), iEle->full5x5_r9(), fabs(iEle->eta()), et, gainSeedSC);  
    double smear = correctionRetriever_.getSmearingSigma(runNumber, iEle->isEB(), iEle->full5x5_r9(), fabs(iEle->eta()), et, gainSeedSC, 0., 0.);
  
    float scale_stat_up = scale + correctionRetriever_.ScaleCorrectionUncertainty(runNumber, iEle->isEB(), iEle->full5x5_r9(), fabs(iEle->eta()), et, gainSeedSC, 1);
    float scale_stat_dn = scale - correctionRetriever_.ScaleCorrectionUncertainty(runNumber, iEle->isEB(), iEle->full5x5_r9(), fabs(iEle->eta()), et, gainSeedSC, 1);
    float scale_syst_up = scale + correctionRetriever_.ScaleCorrectionUncertainty(runNumber, iEle->isEB(), iEle->full5x5_r9(), fabs(iEle->eta()), et, gainSeedSC, 2);
    float scale_syst_dn = scale - correctionRetriever_.ScaleCorrectionUncertainty(runNumber, iEle->isEB(), iEle->full5x5_r9(), fabs(iEle->eta()), et, gainSeedSC, 2);
    float scale_gain_up = scale + correctionRetriever_.ScaleCorrectionUncertainty(runNumber, iEle->isEB(), iEle->full5x5_r9(), fabs(iEle->eta()), et, gainSeedSC, 4);
    float scale_gain_dn = scale - correctionRetriever_.ScaleCorrectionUncertainty(runNumber, iEle->isEB(), iEle->full5x5_r9(), fabs(iEle->eta()), et, gainSeedSC, 4);
    float resol_rho_up  = correctionRetriever_.getSmearingSigma(runNumber, iEle->isEB(), iEle->full5x5_r9(), fabs(iEle->eta()), et, gainSeedSC, 1., 0.);
    float resol_rho_dn  = correctionRetriever_.getSmearingSigma(runNumber, iEle->isEB(), iEle->full5x5_r9(), fabs(iEle->eta()), et, gainSeedSC, -1., 0.);
    float resol_phi_up  = correctionRetriever_.getSmearingSigma(runNumber, iEle->isEB(), iEle->full5x5_r9(), fabs(iEle->eta()), et, gainSeedSC, 0., 1.);
    float resol_phi_dn = correctionRetriever_.getSmearingSigma(runNumber, iEle->isEB(), iEle->full5x5_r9(), fabs(iEle->eta()), et, gainSeedSC, 0., -1.);
