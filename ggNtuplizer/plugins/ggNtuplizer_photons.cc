...
void ggNtuplizer::fillPhotons(const edm::Event& e, const edm::EventSetup& es) {
  ...
  */
  ...
  for (edm::View<pat::Photon>::const_iterator iPho = photonHandle->begin(); iPho != photonHandle->end(); ++iPho) {
    ...
    double scale = correctionRetriever_.ScaleCorrection(runNumber, iPho->isEB(), iPho->full5x5_r9(), fabs(iPho->eta()), et, gainSeedSC);  
    double smear = correctionRetriever_.getSmearingSigma(runNumber, iPho->isEB(), iPho->full5x5_r9(), fabs(iPho->eta()), et, gainSeedSC, 0., 0.);
  
    float scale_stat_up = scale + correctionRetriever_.ScaleCorrectionUncertainty(runNumber, iPho->isEB(), iPho->full5x5_r9(), fabs(iPho->eta()), et, gainSeedSC, 1);
    float scale_stat_dn = scale - correctionRetriever_.ScaleCorrectionUncertainty(runNumber, iPho->isEB(), iPho->full5x5_r9(), fabs(iPho->eta()), et, gainSeedSC, 1);
    float scale_syst_up = scale + correctionRetriever_.ScaleCorrectionUncertainty(runNumber, iPho->isEB(), iPho->full5x5_r9(), fabs(iPho->eta()), et, gainSeedSC, 2);
    float scale_syst_dn = scale - correctionRetriever_.ScaleCorrectionUncertainty(runNumber, iPho->isEB(), iPho->full5x5_r9(), fabs(iPho->eta()), et, gainSeedSC, 2);
    float scale_gain_up = scale + correctionRetriever_.ScaleCorrectionUncertainty(runNumber, iPho->isEB(), iPho->full5x5_r9(), fabs(iPho->eta()), et, gainSeedSC, 4);
    float scale_gain_dn = scale - correctionRetriever_.ScaleCorrectionUncertainty(runNumber, iPho->isEB(), iPho->full5x5_r9(), fabs(iPho->eta()), et, gainSeedSC, 4);
    float resol_rho_up  = correctionRetriever_.getSmearingSigma(runNumber, iPho->isEB(), iPho->full5x5_r9(), fabs(iPho->eta()), et, gainSeedSC, 1., 0.);
    float resol_rho_dn  = correctionRetriever_.getSmearingSigma(runNumber, iPho->isEB(), iPho->full5x5_r9(), fabs(iPho->eta()), et, gainSeedSC, -1., 0.);
    float resol_phi_up  = correctionRetriever_.getSmearingSigma(runNumber, iPho->isEB(), iPho->full5x5_r9(), fabs(iPho->eta()), et, gainSeedSC, 0., 1.);
    float resol_phi_dn = correctionRetriever_.getSmearingSigma(runNumber, iPho->isEB(), iPho->full5x5_r9(), fabs(iPho->eta()), et, gainSeedSC, 0., -1.);
