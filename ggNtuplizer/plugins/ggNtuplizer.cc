...
ggNtuplizer::ggNtuplizer(const edm::ParameterSet& ps) {
  ...
  tree_->Branch("eleSigmaIEtaIEta2012", &eleSigmaIEtaIEta2012_);
  ...
  tree_->Branch("phoSigmaIEtaIEta2012", &phoSigmaIEtaIEta2012_);
  tree_->Branch("phoSigmaIEtaIPhi2012", &phoSigmaIEtaIPhi2012_);
  tree_->Branch("phoSigmaIPhiIPhi2012", &phoSigmaIPhiIPhi2012_);
  tree_->Branch("phoE1x32012",          &phoE1x32012_);
  tree_->Branch("phoE2x22012",          &phoE2x22012_);
  tree_->Branch("phoE2x5Max2012",       &phoE2x5Max2012_);
  tree_->Branch("phoE5x52012",          &phoE5x52012_);
...
void ggNtuplizer::analyze(const edm::Event& e, const edm::EventSetup& es) {
  ...
  if (electronHandle_.isValid()) {
    for (View<pat::Electron>::const_iterator iEle = electronHandle_->begin(); iEle != electronHandle_->end(); ++iEle) {
      ...
      eleSigmaIEtaIEta2012_ .push_back(seeEle);
  ...
  if (photonHandle_.isValid()) {
    for (View<pat::Photon>::const_iterator iPho = photonHandle_->begin(); iPho != photonHandle_->end(); ++iPho) {
      ...
      phoSigmaIEtaIEta2012_ .push_back(see);
      phoSigmaIEtaIPhi2012_ .push_back(sep);
      phoSigmaIPhiIPhi2012_ .push_back(spp);
      phoE1x32012_          .push_back(lazyToolnoZS->e1x3(*((*iPho).superCluster()->seed())));
      phoE2x22012_          .push_back(lazyToolnoZS->e2x2(*((*iPho).superCluster()->seed())));
      phoE2x5Max2012_       .push_back(lazyToolnoZS->e2x5Max(*((*iPho).superCluster()->seed())));
      phoE5x52012_          .push_back(lazyToolnoZS->e5x5(*((*iPho).superCluster()->seed())));
...
void ggNtuplizer::clearVectors() {
  ...
  phoSigmaIEtaIEta2012_.clear();
  phoSigmaIEtaIPhi2012_.clear();
  phoSigmaIPhiIPhi2012_.clear();
  phoE1x32012_.clear();
  phoE2x22012_.clear();
  phoE2x5Max2012_.clear();
  phoE5x52012_.clear();
  phoPFChIso_.clear();
  phoPFPhoIso_.clear();
  phoPFNeuIso_.clear();
