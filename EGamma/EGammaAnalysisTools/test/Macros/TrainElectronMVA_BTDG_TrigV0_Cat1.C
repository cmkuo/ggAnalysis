...
void TrainElectronMVA_BTDG_TrigV0_Cat1() {
  ...
  TFile* inputSignalAll = TFile::Open("./newtmva_Em_5Apr_Zmc.LooseTrig.Real.All.root");   
  TFile* inputBkgAll = TFile::Open("./newtmva_Em_5Apr_Trig.LooseTrig.Fakes.All.root");
  ...
  factory->PrepareTrainingAndTestTree("pt >= 10 && pt < 20 && abs(eta) <= 0.8 && abs(dz) < 0.1",
                                      "pt >= 10 && pt < 20 && abs(eta) <= 0.8 && abs(dz) < 0.1",
